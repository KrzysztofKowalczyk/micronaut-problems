...
public class DefaultHttpClient_hack implements RxWebSocketClient, RxHttpClient, RxStreamingHttpClient, RxSseClient, Closeable, AutoCloseable {
     ...
     */
    public DefaultHttpClient_hack(@Parameter LoadBalancer loadBalancer,
     ...
     * @param filters               The filters to use
     */
    public DefaultHttpClient_hack(URL url,
     ...
     * @param loadBalancer The {@link LoadBalancer} to use for selecting servers
     */
    public DefaultHttpClient_hack(LoadBalancer loadBalancer) {
        this(loadBalancer,
     ...
     * @param url The URL
     */
    public DefaultHttpClient_hack(@Parameter URL url) {
     ...
     * @param configuration The {@link HttpClientConfiguration} object
     */
    public DefaultHttpClient_hack(URL url, HttpClientConfiguration configuration) {
        this(
     ...
     * @param contextPath   The base URI to prepend to request uris
     */
    public DefaultHttpClient_hack(URL url, HttpClientConfiguration configuration, String contextPath) {
        this(
     ...
     * @param configuration The {@link HttpClientConfiguration} object
     */
    public DefaultHttpClient_hack(LoadBalancer loadBalancer, HttpClientConfiguration configuration) {
        this(loadBalancer,
     ...
     * @param contextPath   The base URI to prepend to request uris
     */
    public DefaultHttpClient_hack(LoadBalancer loadBalancer, HttpClientConfiguration configuration, String contextPath) {
        this(loadBalancer,
    ...
    public BlockingHttpClient toBlocking() {
        return new BlockingHttpClient() {
            ...
            public <I, O, E> io.micronaut.http.HttpResponse<O> exchange(io.micronaut.http.HttpRequest<I> request, Argument<O> bodyType, Argument<E> errorType) {
                Flowable<io.micronaut.http.HttpResponse<O>> publisher = DefaultHttpClient_hack.this.exchange(request, bodyType, errorType);
    ...
    protected void configureProxy(ChannelPipeline pipeline, Type proxyType, SocketAddress proxyAddress) {
        ...
        if (StringUtils.isNotEmpty(username) && StringUtils.isNotEmpty(password)) {
            ...
            switch (proxyType) {
                case HTTP:
                    HttpProxyHandler httpProxyHandler = new HttpProxyHandler(proxyAddress);
                    if (pipeline.get(HANDLER_SSL) != null) {
                        pipeline.addBefore(HANDLER_SSL, HANDLER_HTTP_PROXY, httpProxyHandler);
                    } else {
                        pipeline.addLast(HANDLER_HTTP_PROXY, httpProxyHandler);
                    }
                    break;
                case SOCKS:
